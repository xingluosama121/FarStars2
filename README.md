...
- **Trinity rollout (v2.2.0) — meta-framework / framework / product distribution on one kernel** — the three states of the Trinity Architecture Book are now reachable entries: the **meta-framework state** exposes `norpagent.core` (slot table / registry / event bus / address functions) plus the registration trio `register_slot() / register_layer() / register_hook()` and a bare assembly path; the **framework state** keeps the `npa()` SDK, presets, plugins, hooks, FLOW, CNB SDK; the **product state** is launched by **`norpagent unbox`** (one command to a ready-to-use self-evolving app: one powerful agent with **all built-in tools mounted**, a web console, a declarative profile at `~/.norpagent/unbox.json`), implemented in the standalone entry module `norpagent.farstars_app/` (separated from the frame base; `norpagent.unbox` stays as a compatibility forwarder). New in the same release: a **kernel settings store with the full moving-point catalog** (eleven domains) over **four-layer inheritance** (global > profile > session/task > temp; empty = inherit; default / inherited / explicit are all queryable), a **three-view settings panel** (product / framework / meta-framework) with JSON advanced mode, hover explanations, master-switch gating and a directory picker, the **`norpagent settings` CLI** (Web / CLI / REST share one store), a **white-box traversal** (`norpagent.whitebox`) rendering the describe / audit / settings contract for every environment with a console "White-box overview" page, an **evolution loop completion** (proposal engine + proposal center + circuit breaker + 2A memory / 2B skill / 2C config evolvers on top of the 2D code hotswap pipeline), **runtime CNB hot mount** (`npa.remount(cnb=...)` to mount / replace / detach the neural bus while running), and a **frontend V2** (left-tab settings layout, trilingual shared i18n core `/assets/i18n.js` used by the main frontend / orbit console / FLOW page, chat batch-clear, inline mode / model / workspace controls in the composer, Reasoning-effort `xhigh` tier, dependency-aware disabling of dependent options); Post-activation hardening (2026-09-12): code-evolution health checks with automatic revert, active-version sweeps and startup sweeps (`verify_active` / `health_sweep`).

- **Plugin system overhaul (v2.1.0)** — the extension surface closest to the kernel, rebuilt end to end: the kernel hook dispatch is fixed (single mutating dispatch; `intercept` traverses every subscriber and the first non-None rewrite wins — no more double dispatch, no more multi-plugin truncation); the legacy bridge is fully repaired (`PluginContext.logger / storage`, parameter-aligned 16 compatible hooks with an adaptive `on_task_stopped`, dual-key `on_usage_update`, pass-through normalization, visible hook diagnostics); **all 29 hooks** are open to plugins; a **`setup(api)` registration facade** lets plugins register dynamic tools / custom slots / components / models / sessions / sandboxes / schedulers / UIs / custom hooks / event subscriptions / services / Web pages / CLI commands / settings (capability-gated via `PLUGIN_CAPABILITIES`); **lifecycle & clean hot reload** (`on_load` / `on_unload`, real unsubscribe on unload, per-plugin `reload`); dependency and version declarations (`PLUGIN_REQUIRES` / `PLUGIN_MIN_NORPAGENT`); an upgraded **Web plugin panel** (full status / failure reasons / signature / isolation / warnings / diagnostics; enable, disable, uninstall, upload-install) and the CLI `norpagent plugins list|run`; and a new **Chapter 33 "The Complete Plugin Development Guide"** in the developer manual (both languages);
...
- **Self-developed async scheduling core** — `norpagent.nasyncio` (originally `nasync_io`, packaged into the library) is the default event loop core — it neither depends on nor imports the standard `asyncio`; It does not conflict with the standard `asyncio`: both can be used side by side in the same process (the manual's 4.7 and FAQ carry the coexistence statement).
...
- **External plugins** — a complete security pipeline (signature → audit → import restrictions → registration), process-level isolation, the `PluginSystem` facade, all 29 hooks, the `setup(api)` registration surface, lifecycle and clean hot reload (see Chapter 33 of the developer manual);
...
npa(prompt="Briefly explain what an address function is")
running = True
while running:
    if npa.stop() == True:
        running = False
...
npa(model="openai_compat")                              # swap the model
