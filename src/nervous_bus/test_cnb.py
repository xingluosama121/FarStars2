...
覆盖需求点：
  R1 树状拓扑链：每节点唯一父节点，根为中枢；深层注册沿树逐级上报
  R2 分层等级：level 越小等级越高；子级必须大于父级；等级不可篡改
  R3 上行只读：低层级只能 report.*，控制字段被总线拒绝
  R4 下行服从：中枢可对任意层级任意节点下发指令，节点无条件执行
  R5 权限控制：中枢可对任意层级任意单位原子 grant/revoke/set 操作权限
  R6 越权拦截：非祖先的下行拒绝；非后代的 uplink 拒绝；低等级不可改写高等级
  R7 权限面审计上行：perm.denied / perm.changed 经父链
     汇聚中枢 reports，中枢 ctrl op=perm_audit 提供统一权限面审计视图
...
def main():
    ...
    cortex.start()
    check("T01 中枢上线", cortex._bus is not None)
    h = client.health("http://127.0.0.1:17800")
    check("T02 中枢健康检查", h.get("ok") and h.get("node_id") == "cortex"
          and h.get("level") == 0, str(h))

    # ---- 树状拓扑链：中枢 -> bot -> pilot；中枢 -> memory ----
    ...
    # R2：分层等级
    check("T10 中枢 level=0 最高", cortex.level == 0)
    check("T11 bot level=3 > 中枢", bot.level > cortex.level)
