...
def main():
    ...
    check("D09b 中枢收到自定义 status/extra 字段",
    ...
    check("D06d 被救节点心跳续传（rnd 直连中枢）",
          wait_until(lambda: any(r.get("from") == "rnd"
                                 and r.get("type") == "report.heartbeat"
                                 for r in cortex.get_reports())))
    check("D06e 中枢仍可探活整支", cortex.ping("rnd").get("ok")
    ...
    check("D07b 中枢清扫识别 tech2 失联并救树（rnd2 提升挂中枢）",
    ...
    check("D07d 被救 rnd2 心跳恢复上报中枢",
          wait_until(lambda: any(r.get("from") == "rnd2"
                                 and r.get("type") == "report.heartbeat"
                                 for r in cortex.get_reports()), timeout=10.0))
    check("D07e 中枢探活被救整支", cortex.ping("rnd2").get("ok")
          and cortex.ping("dev2").get("ok"))

    # ---- D11: 心跳被拒 -> 自动重新注册（父/中枢视图重建后自愈） ----
    cortex.topology.unregister("dev")  # 模拟中枢视图异常丢失（竞争/误删）
    check("D11a 中枢拓扑已移除 dev（构造失忆场景）",
    ...
    check("D11c 重挂后中枢可探活 dev", cortex.ping("dev").get("ok"))
    ...
    # ---- D13: 中枢清扫后中间层本地缓存收敛（缺口 A） ----
    # 叶子 probe-x 直挂中间层 rnd。强杀（无 deregister）后中枢清扫注销并
    ...
    probe = NervousNode(node_id="probe-x", kind="node", level=5,
                        ...
                        heartbeat_interval=0.4)
    probe.start()
    check("D13a probe-x 注册完成（中枢 + 中间层 rnd 双视图可见）",
    ...
    check("D13f rnd 子树视图与中枢一致（同 id 集合）",
