...
KERNEL_SCHEMA: List[Dict[str, Any]] = [
    ...
    _s("model.context_token_budget", "Context token budget", "model", type="number",
       default=0, minimum=0,
       description="Clamp the request to this estimated token budget by dropping the oldest whole turns (tool_call / tool_call_id pairs are never split); 0 = unlimited",
    ...
    _s("cnb.enabled", "CNB enable switch", "cnb", type="switch", default=False,
       description="Off by default; enable explicitly (R-023); the port must be set manually when enabled (R-025)",
       danger=True),
    ...
    _s("cnb.port", "Node port", "cnb", type="number", default=None,
       description="This node's bus port (never hardcoded; required when enabled, a missing port raises R-025)",
    ...
    _s("cnb.slots_per_node", "Node slot limit", "cnb", type="number", default=64,
       description="Generic slot limit per node (R-024; fixed cap)", locked=True,
    ...
    _s("security.level", "Security level", "security", type="enum", default="standard",
       description="norpagent.safe() protection level: basic / standard / high (applied to the always-on suite)",
    ...
    _s("security.jailbreak_action", "Violation handling", "security", type="enum",
       default="block", description="Action taken when a jailbreak signature is hit",
       config_key="jailbreak_guard_action", options=["block", "warn"],
       enable_when=_on({"key": "security.enabled", "in": [True]},
                       {"key": "security.safe_enabled", "in": [True]},
    ...
    _s("security.unsigned_plugin_policy", "Unsigned-plugin policy", "security",
       ...
       description="Warn only for unsigned plugins, do not block by default (R-020)",
       options=["warn", "block"],
       enable_when=_on({"key": "security.enabled", "in": [True]})),
    _s("security.safe_mode", "Safe mode", "security", type="switch",
       default=False, locked=True,
       description="Safe-mode startup: minimal kernel only; no slot loads external code (R-015)",
       danger=True),
    ...
    _s("plugins.security_audit", "Load audit", "plugins", type="enum",
       default="block", description="Plugin security-audit level",
       config_key="plugin_security_audit", options=["off", "warn", "block"],
       enable_when=_on({"key": "plugins.enabled", "in": [True]})),
    _s("plugins.import_restrict", "Import restrictions", "plugins", type="enum",
       default="strict", description="Plugin import allowlist restriction level",
       config_key="plugin_security_import_restrict",
       options=["off", "safe", "strict"],
       enable_when=_on({"key": "plugins.enabled", "in": [True]})),
    ...
    _s("evolution.enabled", "Self-evolution master switch", "evolution", type="switch",
       default=True, description="When off, all evolvers stop (R-005 / R-011)"),
    ...
    _s("evolution.idle_min_seconds", "Idle threshold", "evolution", type="number",
       default=600, description="No demand for this duration counts as idle (R-011)", minimum=0,
    ...
    _s("evolution.candidate_threshold", "Evolution candidate trigger count", "evolution",
       ...
       description="Frequently used features (>=3 times) become evolution candidates (R-011)"),
